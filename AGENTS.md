...
> 本文档供 AI 智能体（Codex）在后续会话中恢复项目上下文。
> 每次重大变更后应更新此文件。
...
### 3. `yiduan.db`
**来源**：2020–2025 年一分一段表（分数-位次对照），2020–2024 来自 WPS 文字 PDF（pdftotext -raw），2025 来自扫描 PDF（kimi-k2.6 OCR）。

**表 `yiduan`**（1,803 条）：`year, score, count（该分数人数）, cumulative（累计排名）`。
...
8. **阳光高考网**：`gaokao.chsi.com.cn` 有反爬保护（JS challenge），2026 年招生计划预计 6 月 23 日前后公布
